...
public:
    Task(int cpuNum, int size){
        this->cpuNum = cpuNum; // CPU core number of the task
        this->size = size; // size of the task
    }
...
class Analyzer{
private:
    vector<int> coreLoads; // Vector to store CPU loads
public:
    Analyzer(int numCores)
{
        coreLoads.resize(numCores, 0);// Resize and initialize cpuLoads vector with zeros
    }
